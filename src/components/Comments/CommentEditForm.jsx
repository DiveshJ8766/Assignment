...
const CommentEditForm = ({ comment, onSave, onCancel, IS_DOC_REQUIRED }) => {
    const [text, setText] = useState(comment.text);
    const [fieldLabel, setFieldLabel] = useState(comment.fieldLabel || "Field value");
    const [file, setFile] = useState(comment.file);
    ...
    const handleSave = () => {
        ...
        if (!text.trim()) newErrors.comment = "Comment is required";
        if (IS_DOC_REQUIRED && !file) newErrors.file = "Supporting document is required";
        ...
        onSave(comment.id, text, file, fieldLabel);
    };
    ...
    return (
        <div className="bg-white dark:bg-[#3b425e] p-4 rounded-xl border border-orange-300 dark:border-slate-600 shadow-md animate-in fade-in zoom-in-95 duration-200 transition-all duration-300">
            <div className="space-y-4">
                ...
                {/* Action Buttons: Discard & Submit Suggestion */}
